    ...
    for t in ticker_df[ticker_df.peer_group == peer_group - 1]["ticker"].values:
        ...
        try:
            ...
            try:
                ...
                recommendation_table = soup.find(
...
def create_peer_df(ticker: str, levels: int = 3) -> pd.DataFrame:
    ...
    """
    ticker_dict = {"ticker": [ticker], "peer_group": [0]}

    ticker_dataframe = pd.DataFrame.from_dict(ticker_dict)
    for i in range(1, levels):
        ticker_dataframe = get_linked_tickers(
            ticker_df_in=ticker_dataframe, peer_group=i
        )
        ticker_dataframe = (
            ticker_dataframe.groupby("ticker")["peer_group"].min().reset_index()
        )

    ticker_dataframe = (
        ticker_dataframe.drop(columns=["peer_group"])
        .drop_duplicates()

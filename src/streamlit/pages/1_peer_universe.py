...
def main():
    ...
    if company_ticker != "":
        st.session_state["main_ticker"] = company_ticker # Making sure that the primary ticker is remembered until it is changed.
    ...
    if st.button("Set Primary Ticker", key="start_key") or st.session_state["main_ticker"]!="":
        ...
        cols = st.columns(number_of_peers) # Adapting according to how many peers one wish to include.
        for i in range(number_of_peers):
            col = cols[i]
            peer=col.text_input(label="", value="", key=i)
            peer_list.append(peer)

        st.session_state["peer_list"] = peer_list # Remembering the list of peers for the analysis page.
        ...
        if st.button("Add Peers", key="add_peers"):
            ...
            yahoo_extract_progress = st.progress(0, text=progress_text) # If users move too fast the data won't be stored.
            ...
            for i, peer in enumerate(peer_list):
                ...
                progress = (i+1.0) / (number_of_peers+1.) # Adding one so the final step is when the data is stored
                yahoo_extract_progress.progress(progress, text=progress_text)

            st.session_state["data"] = full_df # Storing the data for the analysis page.
